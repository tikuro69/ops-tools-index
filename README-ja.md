...
このツール群は、小規模なLinuxサーバ運用、障害初動調査、設定確認を支援するためのものです。

1. **initial_triage_tool**  
   サーバの基本情報を収集し、初動調査用のレポートを作成します。

2. **log_parser_tool**  
   Apache/Nginx のアクセスログを要約し、アクセス傾向や異常の手がかりを確認します。

3. **nginx_apache_config_linter**  
   Webサーバ設定ファイルを確認し、よくある設定ミスや注意点を洗い出します。

4. **mail_header_probe**  
   メールヘッダーを解析し、SPF/DKIM/DMARC などの認証情報を確認します。

5. **iptables-rule-viewer**  
   iptables のルールを見やすく整理し、ファイアウォールの挙動を把握しやすくします。

## コンセプト

これらのツールは、運用担当者が障害対応時に必要な情報を素早く集め、人間またはAIアシスタントが確認しやすい形に整理することを目的としています。
